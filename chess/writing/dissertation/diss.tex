...
Haskell is a purely functional language, based upon lambda calculus with the addition of a type system\footnote{\url{https://www.haskell.org/}}. All Haskell expressions have a type, which is inferred at compile time. Programs go through \emph{type erasure}---the executable output of the compiler has no notion of types. In other words, Haskell has a clear separation between dynamic values, and the static types those values are members of.

As such, passing information between the type system and the runtime values (as is common in dependently typed languages) is made more difficult. This section summarises key aspects of Haskell's type system, including how programmers can use GHC extensions and advanced features to perform computation at the type level in Haskell.

\emph{A short note}: Haskell has an interactive Read-Eval-Print-Loop interpreter, named GHCi. The below chapter contains several GHCi snippets, to aid in understanding. \inline{\$>} represents a terminal input line, and anything following it can be assumed to have been typed into GHCi. A programmer can query the type of a value with \inline{:t}. So, \inline{\$> :t True} denotes an attempt to discover the type of the value \inline{True}, which is of course \inline{Bool}. Additionally, \inline{::} means "has type", so \inline{3 :: Int} reads as "the value \inline{3} has type \inline{Int}".
