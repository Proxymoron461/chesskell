...
% For values that have been promoted to types, a ' may be prefixed (as with \lstinline[basicstyle=\ttfamily]{'Black}) above. However, this prefix is optional, since GHC can, in most cases, tell whether something is a value or a type from context. Where unclear, this report will use the prefix to distinguish types from values.
...
% The below definition makes use of \emph{Generalised Algebraic Data Type} syntax, or GADT syntax for short. This is an extension to Haskell, allowing programmers to define more complex data types by describing the types of its' type constructors. This is clear below: \lstinline[basicstyle=\ttfamily]{Z} is a value of type \lstinline[basicstyle=\ttfamily]{Nat}, while \lstinline[basicstyle=\ttfamily]{S} is a function that takes in a \lstinline[basicstyle=\ttfamily]{Nat} and returns another \lstinline[basicstyle=\ttfamily]{Nat}.
...
% The Position type itself is a \emph{product type}, a type whose cardinality\footnote{A type's cardinality is the number of inhabitants it has.} is equal to the product of the types it takes in. Since \lstinline[basicstyle=\ttfamily]{Position} takes in a type of cardinality 8 (\lstinline[basicstyle=\ttfamily]{Column}) and a type of infinite cardinality (\lstinline[basicstyle=\ttfamily]{Nat}), it has infinite cardinality, even though only 64 of them are valid chess positions.
...
The EDSL function \lstinline[basicstyle=\ttfamily]{becomes} handled Pawn promotion in earlier versions of Chesskell, transforming a Pawn into another piece after it has moved to the opposite side of the board. Since the whole chess game is expressed in continuation-passing style, \lstinline[basicstyle=\ttfamily]{becomes} simply edits a Pawn's \lstinline[basicstyle=\ttfamily]{PieceName} in place. %, with the help of type family \lstinline[basicstyle=\ttfamily]{PromotePieceTo}.
