...
type WhitePawnFirst = FromJust' (Eval (Move (At A Nat2) (At A Nat4) StartBoard))
enPassantTest1 :: True :~: Eval (IsSpaceVulnerableToEnPassant Black WhitePawnFirst (At A Nat4))
enPassantTest1 = Refl

enPassantTest2 :: False :~: Eval (IsSpaceVulnerableToEnPassant Black (FromJust' (Eval (Move (At A Nat2) (At A Nat3) StartBoard))) (At A Nat3))
enPassantTest2 = Refl

type BlackPawnFirst = FromJust' (Eval (Move (At A Nat7) (At A Nat5) StartBoard))
enPassantTest3 :: True :~: Eval (IsSpaceVulnerableToEnPassant White BlackPawnFirst (At A Nat5))
enPassantTest3 = Refl

enPassantTest4 :: False :~: Eval (IsSpaceVulnerableToEnPassant White (FromJust' (Eval (Move (At A Nat7) (At A Nat6) StartBoard))) (At A Nat6))
enPassantTest4 = Refl
